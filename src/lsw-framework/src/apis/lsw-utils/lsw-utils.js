(function (factory) {
  ...
  LswUtils.arrayToMarkdownTable = function (arr, order = []) {
    if (!Array.isArray(arr) || arr.length === 0) {
      return '';
    }
    // Obtener todas las claves de todos los objetos
    const allKeys = [...new Set(arr.flatMap(obj => Object.keys(obj)))];
    // Reordenar: primero las del order[], luego el resto
    const orderedKeys = [...order, ...allKeys.filter(k => !order.includes(k))];
    // Escapar valores para Markdown
    const escapeValue = v =>
    ...
    const rows = arr.map(obj =>
      '| ' + orderedKeys.map(k => escapeValue(obj[k])).join(' | ') + ' |'
    );
